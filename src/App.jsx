...
import { LogOut, BookOpen, Apple, Sparkles, Plus, Star } from "lucide-react"
import "./App.css"

function App() {
  const [session, setSession] = useState(null)
  const [loading, setLoading] = useState(true)
  const [viewState, setViewState] = useState("list") // list, create, mybooks, viewer
  ...
  const handleShowMyBooks = () => {
    setViewState("mybooks")
    setSelectedBookId(null)
  }

  const handleCreateComplete = () => {
    setViewState("mybooks") // Go to my books to see the new one
  }
      ...
      <header className="app-header !bg-white/90 sticky top-0 z-50 backdrop-blur-2xl border-b-8 border-[#ccff00] !shadow-2xl py-6 transition-all">
        <div className="container px-8 flex justify-between items-center">
          ...
          <div className="header-actions flex items-center gap-6">
            {viewState !== "create" && viewState !== "mybooks" && !selectedBookId && (
              <>
                <button 
                  onClick={handleShowMyBooks} 
                  className="hidden lg:flex items-center gap-3 px-8 py-3 bg-[#f0ffcc] text-[#2d4a22] font-black rounded-full hover:bg-[#ccff00] transition-all border-2 border-[#ccff00] shadow-md hover:shadow-lg active:translate-y-0.5"
                >
                ...
                <button 
                  ...
                  className="flex items-center gap-3 px-10 py-4 bg-[#2d4a22] text-[#ccff00] font-black rounded-[2rem] shadow-[0_8px_0_#1a2e14] hover:shadow-[0_4px_0_#1a2e14] hover:translate-y-1 active:translate-y-2 transition-all text-lg"
                >
            ...
            {(selectedBookId || viewState === "create" || viewState === "mybooks") && (
              <button 
                onClick={handleBackToList} 
                className="px-10 py-3 bg-white text-[#2d4a22] font-black rounded-full border-[6px] border-[#ccff00] hover:bg-[#f7fff0] transition-colors shadow-lg active:translate-y-1"
              >
                 もどる
              </button>
            )}
            ...
            <button 
              ...
              className="p-3 text-[#2d4a22] opacity-30 hover:opacity-100 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all"
              title="ログアウト"
            >
